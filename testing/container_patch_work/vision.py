    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.qk_normalization and not self.qk_normalization_by_head_size:
            local_norm_dim = head * self.head_size
            norm_hidden_size = getattr(self.q_norm, "hidden_size", local_norm_dim)
            if self.tp_size > 1 and norm_hidden_size != local_norm_dim:
                q, k = self._apply_qk_norm(q, k)
            # jit kernel
            elif can_use_jit_qk_norm(self.head_size, q.dtype):

                # q: [tokens, head, head_size]  ->  [tokens, embed_dim]
                head_dim_for_norm = local_norm_dim
