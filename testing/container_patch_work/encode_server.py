...
def _convert(data):
    ...
    elif isinstance(data, list) and data and isinstance(data[0], torch.Tensor):
        try:
...
class MMEncoder:
    ...
    async def _process_mm_items(self, mm_items, modality):
        if modality == Modality.IMAGE and self.model_type == "internvl_chat":
            ...
            img_max_num = int(
                image_config.get(
                    "max_dynamic_patch",
                    getattr(self.model_config.hf_config, "max_dynamic_patch", 12),
                )
            )
            image_size = (
                getattr(self.model_config.hf_config, "force_image_size", None)
                or self.model_config.hf_config.vision_config.image_size
            )
            patch_size = self.model_config.hf_config.vision_config.patch_size
            if isinstance(image_size, list):
                image_size = image_size[0]
            if isinstance(patch_size, list):
                patch_size = patch_size[0]
            use_thumbnail = bool(
                getattr(self.model_config.hf_config, "use_thumbnail", True)
            )
            grid_hw = int(image_size) // int(patch_size)

            mean, std = InternVLProcessor._get_normalize_tensors(device=self.device)
            pixel_values_list = []
            image_grid_thw = []
            for image in images:
                if isinstance(image, Image.Image):
                    ...
                    tensor = image.to(self.device).float()

                tensor = (tensor - mean) / std
                tiles = InternVLProcessor.dynamic_preprocess(
                    tensor,
                    image_size=int(image_size),
                    max_num=max(1, img_max_num),
                    use_thumbnail=use_thumbnail,
                )
                pixel_values_list.append(tiles)
                image_grid_thw.append([int(tiles.shape[0]), grid_hw, grid_hw])

            processor_input = {
                "pixel_values": torch.cat(pixel_values_list, dim=0),
                "image_grid_thw": torch.tensor(
                    image_grid_thw, dtype=torch.long, device=self.device
                ),
            }
            get_feature_method = self.model.get_image_feature
        elif modality == Modality.IMAGE and self.image_processor:
            images = await self._flatten_and_load_images(mm_items)
            image_config = self.vision_config.get("image", {})
